...
export const ProductsDetails: React.FC<ProductsDetailsProps> = ({ product }) => {
  ...
  const decrement = () => setLocalQty(prev => (prev > 1 ? prev - 1 : 1));
    ...
    <div className="md:flex md:flex-row-reverse md:gap-[80px] xl:items-center md:items-center justify-between max-w-[1370px] w-full my-0 mx-auto px-[15px] py-10">
      ...
      <div className="bg-[#F5F7FF] md:w-[424px] xl:w-[600px] flex-1 p-[20px] rounded-lg">
        <div className="flex gap-[17px] mb-[13px] md:mb-[25px]">
          {tabs.map(t => (
            <button
              ...
              className={classNames('font-poppins font-semibold text-[#666666] text-[12px] pb-[5px] relative xl:text-[14px]', {
        ...
        <div className="flex justify-between mb-[15px]">
          <button onClick={handleAddToCart} className="font-poppins font-semibold text-[13px] text-white bg-[#0156FF] px-10 py-[12px] rounded-[30px] hover:bg-[#0044cc] transition-colors w-full md:w-auto">
            Add to Cart
          </button>
        </div>
